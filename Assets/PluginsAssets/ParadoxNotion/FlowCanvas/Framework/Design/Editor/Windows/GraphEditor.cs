	...
	public partial class GraphEditor : EditorWindow{
		...
		void OnGUI(){
			...
			if (EditorApplication.isCompiling){
				ShowNotification(new GUIContent("...脚本编译中..."));
				willRepaint = true;
				return;			
			}
			...
			if (Application.isPlaying){
				GUI.Box(canvasRect, string.Empty, CanvasStyles.window_event);
			}else
			{
				GUI.Box(canvasRect, string.Empty, CanvasStyles.window_variable);
			}
			//background grid
		...
		static void DoCanvasGroups(){
			...
			for (var i = 0; i < currentGraph.canvasGroups.Count; i++){
				...
				GUI.color = true? new Color(1,1,1,0.4f) : new Color(0.5f,0.5f,0.5f,0.3f);
