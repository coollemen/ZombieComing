...
namespace TDTK{
	...
	public class Unit : MonoBehaviour {
		...
		public virtual void Update(){
			if(!IsStunned() && IsTurret() && !IsPreview() && !InConstruction()) Aim();
		}
		...
		IEnumerator Shoot(AttackInfo aInfo){
			...
			for(int i=0; i<shootPoint.Count; i++){
				...
				if(i==shootPoint.Count-1) soInstance.InitShoot(aInfo);
				else soInstance.InitShoot(aInfo.tgtUnit);
		...
		public void ApplyAttack(AttackInfo aInfo){
			...
			if(aInfo.UseEffect()) ApplyEffect(aInfo.effect);
		}
