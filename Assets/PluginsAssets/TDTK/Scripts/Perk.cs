	...
	*/
	...
	public class Perk : TDItem{
		...
		public _Status Purchase(bool useRsc=true){
			...
			if(PerkManager.InGameScene()){
				...
				if(type==_PerkType.NewAbility){
					...
					if(!IsMultiplier()) AbilityManager.MultiplyRscCap((int)Mathf.Round(gain));
					else AbilityManager.ModifyRscCap((int)Mathf.Round(gain));
				}
		...
		void SetupCallback(){
			...
			GetOverrideOnHitEff = (prefabID) => 		{ return CheckID(prefabID) ? effect.stats.effectOnHitID : -1; };
