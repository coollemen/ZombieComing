		...
		public bool CheckForNode(int nodeIdx){
			...
			if(branch){
				...
				for(int i=0; i<exitNodeIDList.Count; i++){
					...
					blockSec|=(platform.searchPath(entryNodeID, exitNodeIDList[i], nodeIdx).Count>0);
			...
			if(!blockSec) blockSec=platform.searchPath(entryNodeID, exitNodeID, nodeIdx).Count==0;
			return !blockSec;
		}
