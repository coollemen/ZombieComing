...
namespace FlowCanvas.Nodes{
	...
	public class SimplexNodeWrapper<T> : FlowNode where T:SimplexNode {
        ...
        protected override GUIStyle nodeGUIType()
        {
            if (typeof(T).IsSubclassOf(typeof(PureFunctionNodeBase)))
                return NodeCanvas.Editor.CanvasStyles.window_function;
            else
            {
                return NodeCanvas.Editor.CanvasStyles.window;
            }
        }
#endif
