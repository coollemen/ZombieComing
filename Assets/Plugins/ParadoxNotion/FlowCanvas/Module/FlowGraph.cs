...
namespace FlowCanvas {
	...
	abstract public class FlowGraph : Graph {
		...
		protected override void OnGraphStarted(){
			...
			//SL------------
			if (tempGraphRecordState!=null&&tempGraphRecordState.Count>0)
			{
				tempGraphRecordState.ForEach(x=> {if(x.Value){x.Key.StartGraph(agent,x.Key.useLocalBlackboard?x.Key.localBlackboard:blackboard,false,null);}});
			}
		...
		Dictionary<Graph,bool> tempGraphRecordState=new Dictionary<Graph, bool>();
		//...
		protected override void OnGraphStoped(){
			for (var i = 0; i < allNodes.Count; i++){
				...
				//SL------------
				var allChildGraph= GetAllNestedGraphs<Graph>(true);
				tempGraphRecordState.Clear();
				allChildGraph.ForEach(x=>{tempGraphRecordState.Add(x,x.isRunning); x.Stop();});
			}
		}
