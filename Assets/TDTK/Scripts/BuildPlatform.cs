...
namespace TDTK{
	...
	public class BuildPlatform : MonoBehaviour {
		...
		*/
		...
		public List<Vector3> searchPath(int entryID, int exitID, int blockIdx=-1){
			List<Vector3> list=AStar.Search(nodeGraph[entryID], nodeGraph[exitID], nodeGraph, blockIdx>=0 ? nodeGraph[blockIdx] : null);
